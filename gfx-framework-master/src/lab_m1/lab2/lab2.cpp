...
void Lab2::Init()
{
    ...
    // by definition!
    // Create a tetrahedron mesh using 12 vertices
    {
        ...
        vector<unsigned int> indices_tet =
        {
			0, 1, 2,
			0, 2, 3,
			0, 3, 1,
		...
		meshes["tetrahedron"]->InitFromData(vertices_tet, indices_tet);
	}
    ...
    meshes["cube_bonus"] = new Mesh("generated cube 1");
    meshes["cube_bonus"]->InitFromData(vertices1, indices1);
...
void Lab2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(0, 0.5f, 1.5f), glm::vec3(0.25f));

    // TODO(student): Draw the square
	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(2, 2, 1), glm::vec3(0.25f));

	// render the bonus cube
	RenderMesh(meshes["cube_bonus"], shaders["VertexColor"], glm::vec3(-1, -1, -1), glm::vec3(0.25f));
